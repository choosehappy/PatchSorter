...
if __name__ == '__main__':
    ...
    # args = parser.parse_args(
    #     ["test1", r"C:\temp\PatchSorter\projects\test1\patches_test1.pytable", "5", r"-o./projects/test1/models/1"])
    # args = parser.parse_args(
    #     ["Testing_Int", r"./projects/Testing_Int/patches_Testing_Int.pytable", "3", r"-o./projects/Testing_Int/models/0/"])

    #args = parser.parse_args([ 'chuv_mel', './projects/chuv_mel/patches_chuv_mel.pytable', '2', '-p32', '-n50', '-s-1', '-l10', '-b16', '-o./projects/chumel/models/0/', '-r0'])
    ...
    try:
        ...
        model = Autoencoder(codesize=args.codesize, nclasses=args.nclasses).to(device)
        ...
        if class_weight.sum():  # some labeliong has occred so modify weights
            class_weight = torch.from_numpy(1-class_weight/class_weight.sum()).type('torch.FloatTensor').to(device)
        else:
            class_weight = None

        #criterion2 = nn.CrossEntropyLoss(ignore_index=-1)
        criterion2 = nn.CrossEntropyLoss(ignore_index=-1,reduction='none', weight=class_weight)
        ...
        for epoch in range(num_epochs):
            ...
            for X, img_old, pred, gt, mask in data_train_loader:
                ...
                Xvec = model.features(X)
                pred_class = model.vec2class(Xvec)
                pred_ae = model.vec2img(Xvec)

                lossl1 = torch.sum(torch.abs(Xvec), 1).mean() * .001
                # lossl1 = 0
                lossmse = criterion(pred_ae, X)
                lossclass = criterion2(pred_class, pred)

                lossclass_class_labeled_weighted = 100*(lossclass[gt != -1]).mean()
                lossclass_class_labeled_weighted = 0 if torch.isnan(lossclass_class_labeled_weighted) else lossclass_class_labeled_weighted

                lossclass_class_unlabeled_weighted = (lossclass[gt == -1]).mean()
                lossclass_class_unlabeled_weighted = 0 if torch.isnan(lossclass_class_unlabeled_weighted) else lossclass_class_unlabeled_weighted

                loss = lossl1 + lossmse + lossclass_class_unlabeled_weighted + lossclass_class_labeled_weighted
            ...
            print('%s ([%d/%d] %d%%), loss: %.4f ' % (timeSince(start_time, (epoch + 1) / num_epochs),
                                                      epoch + 1, num_epochs, (epoch + 1) / num_epochs * 100, all_loss),end="",flush=True)
            ...
            if all_loss < best_loss:
                ...
                torch.save(state, f"{newmodeldir}/best_model_autoencoder_C{args.codesize}_N{args.nclasses}_P{args.patchsize}.pth")
