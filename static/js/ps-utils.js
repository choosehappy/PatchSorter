...
function updateJobNotifications(job_id) {
  ...
  // extract the data:
  let messages = new_job_notifications.result;//new_job_notifications.objects;
  let count = new_job_notifications.result.length; //new_job_notifications.num_results;
  //running_jobs_line_number[job_id] = (count == 0 ? 0 : messages[count-1].id);
  running_jobs_line_number[job_id] = (count == 0 ? 0 : messages[count-1][0]);
 ...
 */
function jobChecker(job_id, completed_callback_function, retry_seconds, command_name) {
    ...
    if (job_status == 'QUEUE' || job_status == 'RUNNING') {
        setTimeout(jobChecker.bind(null,
            job_id, completed_callback_function, retry_seconds, command_name),
            1000 * retry_seconds);
    }else {
 ...
 */
function startJobChecker(job_id, command_name, retry_seconds, completed_callback_function=function(){}) {
    if (running_jobs_being_checked.has(job_id)) {
        ...
        running_jobs_line_number[job_id] = Math.max(0, get_largest_id(job_id) - lines_to_start_with); // <-- reset the line number
        ...
        jobChecker(job_id, completed_callback_function, retry_seconds, command_name);
    }
}
////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 */
function startJobCheckerIfExists(project_id, image_id, command_name, completed_callback_function) {
    ...
    if (job_id) {
        ...
        startJobChecker(job_id, command_name, retry_seconds, completed_callback_function);
    }
}
////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 */
function loadRunningJobsForProject(project_id, completed_callback_function = function(){},commands_to_check = ['make_patches', 'train_dl', 'make_embed','make_searchdb']) {
    // commands_to_check = ['make_patches', 'train_dl', 'make_embed'];
    image_id = null;
 ...
 *
 */
function loadObjectAndRetry(url, completed_callback_function=function(){}, ignoreError=false) {
 ...
 * call loadedFunction()
 */
function loadImageAndRetry(url, context, image_loaded_callback, ignore_error=false) {

    // we will create a custom function to draw the image onto the context:
    const on_image_ready = function(url) {
        // console.log("URL IS in loadImageAndRetry "+ url)
        // create a new temporary image:
        let temp_image = new Image();

        temp_image.onload = function() {
            context.drawImage(temp_image, 0, 0, context.canvas.width, context.canvas.height);
            image_loaded_callback(temp_image);
        }
    ...
    loadObjectAndRetry(url, on_image_ready.bind(null, url), ignore_error);
...
function getDatabaseQueryResults(table_name, col_name, operation, value,
    ...
    let response;
    try {
        response = JSON.parse(requester.responseText)
    } catch (e) {
    }
    if (status !== 200) {
 ...
 * -1 if none exist.
 */
function get_largest_id(jobid) {

        
    var job_url = new URL('/api/db/maxjobid',window.location.origin)
    job_url.searchParams.append("job_id", jobid);
    let jobReq = new XMLHttpRequest();
    const async = false;
    jobReq.open("GET", job_url, async);
    jobReq.send();
// handle the response
    const status = jobReq.status;
    let response
    try { response  = JSON.parse(jobReq.response)} catch (e) { }
    ...
    if (status == 200) {
        return response.result[0][0];
            ...
            const error_message = (status === 404) ?
                ('ERROR: jobid_' + jobid + ' not found in database.') :
...
function get_job_notifications(job_id, line_number) {
    ...
    var job_url = new URL('/api/db/jobinfo',window.location.origin)
    job_url.searchParams.append("job_id", job_id);
    job_url.searchParams.append("line_number", line_number);
    let jobReq = new XMLHttpRequest();
    const async = false;
    jobReq.open("GET", job_url, async);
    jobReq.send();
// handle the response
    ...
    if (status != 200) {
        const error_message = (status === 404) ?
                ('ERROR: jobid_' + job_id + ' not found in database.') :
                ('ERROR: ' + status + response.error);
            showWindowMessage(error_message);
    }
...
/**
 * Obtains the current count of uploaded Images.
 * @returns {count of images}
 */
function getImageCount(projectId) {
...
/**
 * Function to enable and disable buttons by ID
 */
function toggleButton(buttonId,toggleVal,title="")
{
    document.getElementById(buttonId).disabled = toggleVal;
    document.getElementById(buttonId).title = title;
}
...
function updateStatistics(projectstats) {
    ...
    percent_dist = projectstats["percent_dist"];
    let val = "";
    if (Object.keys(percent_dist).length>0)
    {
        values = Object.values(percent_dist).join('%, ');
        val = "("+values+"%)";
    }
   document.getElementById("lbl_legend").innerHTML = val;
}
