...
function loadData(jsonData,init_status,action) {
    ...
    if (init_status) {
        //Subtracted 1 from mins and added 1 to max to avoid the points getting to close to the axis.
        curr_xScale = d3.scaleLinear().domain([jsonData["xmin"] - 1, jsonData["xmax"] + 1]).nice().range([0, width]);
        curr_yScale = d3.scaleLinear().domain([jsonData["ymin"] - 1, jsonData["ymax"] + 1]).nice().range([height, 0]);
        ...
        //console.log('initial_xMin', initial_xMin)
        //console.log('initial_xMax', initial_xMax)
        //console.log('initial_yMin', initial_yMin)
        //console.log('initial_yMax', initial_yMax)

    }
    //console.log("X --> " + curr_xScale.domain()[0] + " and ->" + curr_xScale.domain()[1]);
    //console.log("Y --> " + curr_yScale.domain()[0] + " and ->" + curr_yScale.domain()[1]);

    draw_update_plot(orig_plot_data, init_filterby, init_plotby, init_classby, curr_xScale, curr_yScale);
    /**
    ...
    d3.selectAll(("input[name='color_by']")).on("change", function () {
        ...
        draw_update_plot(orig_plot_data, filterby, plotby, classby, curr_xScale, curr_yScale);
    });
    /**
   ...
   d3.select("#filterlabel").on("change", function() {
       ...
       //Filter the plot data
        ...
        let filterby = $("input[name='filter_by']:checked").val()
        let classby = $( "#filterlabel" ).val();
        // cell_data = filter_by(filterby,orig_plot_data);
        d3.select("#patch_svg_group").remove();
        draw_update_plot(orig_plot_data,filterby,plotby,classby,curr_xScale,curr_yScale);
    });

    updateEmbeddingPercent(jsonData["totalres"],jsonData["providedres"]);
    document.getElementById("viewpatches").disabled = false;
...
window.setInterval(
    function(){
        // console.log('last_zoom_time = ', last_zoom_time)
        // console.log('last_redraw_time = ', last_redraw_time)
        // console.log('last_checked_time = ', last_checked_time)

        const needs_redraw = last_redraw_time < last_zoom_time &&
                                                last_zoom_time < last_checked_time
                                                
        // console.log('needs_redraw = ', needs_redraw)
        if (proj_embed_iteration > -2 && needs_redraw) {
            //console.log("Current Action is "+ curr_action);
            if(curr_action == "zoom"){
                ...
                //console.log('current_xMin', current_xMin)
                //console.log('current_xMax', current_xMax)
                //console.log('current_yMin', current_yMin)
                //console.log('current_yMax', current_yMax)
                if (current_xMin < initial_xMin ||
                    current_xMax > initial_xMax ||
                    current_yMin < initial_yMin ||
                    current_yMax > initial_yMax) {

                    //console.log('Not calling initialize_plots')
...
function draw_update_plot(plotdata, filterby, plotby, classby,xScl, yScl) {
    //console.log('draw_update_plot', xScl, yScl)
    ...
    var lasso_area = patch_svg.append("rect")
        .attr("id", "lasso_area")
        .attr("width", width)
        .attr("height", height)
        .style("opacity", 0)
    ...
    function zoomed()
    {
        ...
        //console.log('curr_xScale', curr_xScale())
        //console.log('curr_yScale', curr_yScale)

        new_xScale = transform.rescaleX(xScl);
        new_yScale = transform.rescaleY(yScl);

        //console.log('new_xScale', new_xScale)
        //console.log('new_yScale', new_yScale)
...
var lasso_end = function () {
        ...
        // if (patch_selected != null && patch_selected.length > 0) {
        //     if(DEBUG) {console.log("No of Patch selected are .." + patch_selected.length);}
        // }
        /* New Implementation to pick points by polygon */
        // lassopolygon=document.getElementsByClassName("drawn")[0].getAttribute("d")
        // pagelimit = $( "#no_of_patch" ).val();
        if(lasso_path != "") {
            // console.log("lasso_path" + lasso_path);
             waitCursor();
            /* Retrieve filter elements to filter data */
