...
### 3.1. Batch read of patch data for visualization
How quickly can a random batch of patches be retrieved for display in the patch gallery UI? A batched random read is useful for visualization purposes; DL model training requires contiguous (sequential) reads rather than random batched reads, as shuffling is handled at the DataLoader level.
