...
from model.simclr import info_nce_loss
...
if __name__ == '__main__':
    ...
    #      '-s-1', '-l10', '-b768', '-o./projects/chumel/models/0/',
    #      '-r-1'])
    ...
    try:
        ...
        model = Simclr(codesize=args.codesize, nclasses=args.nclasses).to(device)
        ...
        criterion_simclr = nn.CrossEntropyLoss(ignore_index=-1)
        ...
        for epoch in range(num_epochs):
            ...
            for X1, X2, img_old, pred, gt1, mask in data_train_loader:
                ...
                gt = torch.cat((gt1, gt1), dim=0).to(device)
                gt = gt.type(torch.LongTensor).to(device)

                features = model.features(X)
                logits, labels = info_nce_loss(features=features, device=device,
                                               temperature=args.temperature)
                slr_loss = criterion_simclr(logits, labels)

                pred_class = model.forward_pred(features)

                class_loss = 10 * criterion_pred(pred_class, gt)  # --- convert into weight from config file

                loss = slr_loss + class_loss
            ...
            if all_loss < best_loss:
                ...
                torch.save(state,
                           f"{newmodeldir}/best_model_simclr_C{args.codesize}_N{args.nclasses}_P{args.patchsize}.pth")
