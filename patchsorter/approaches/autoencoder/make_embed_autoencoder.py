...
if __name__ == '__main__':
    ...
    # # +
        ...
        model_name = f"{args.outdir}/best_model_autoencoder_C{args.codesize}_N{args.nclasses}_P{args.patchsize}.pth"
        ...
        all_vecs_full = all_vecs_full.reshape(all_vecs_full.shape[0], all_vecs_full.shape[1], -1)

        all_vecs_full = all_vecs_full.reshape(all_vecs_full.shape[0], -1)
        ...
        print(f"RETVAL: {json.dumps({'project_name': args.project_name, 'modelid': modelid, 'xmax': xmax, 'xmin': xmin, 'ymax': ymax, 'ymin': ymin})}", flush=True)
