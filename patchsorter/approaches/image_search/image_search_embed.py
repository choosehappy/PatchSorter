...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument('-m', '--maxthresh', help="Minimum threshold for positive result", default=np.Inf,
                            type=float)
        ...
        #args = parser.parse_known_args(['lymph', './projects/lymph/patches_lymph.pytable', 'projects\\lymph\\image_search\\lymph_367.png', './projects/lymph/models/0/', '-p32', '-n100', '-m10'])[0]
        ...
        reducer = dill.load(open(f"{model_dir}/umap_model.pkl","rb"))

        if vec_query.shape[1] != reducer._raw_data.shape[1]:   #need to see if dimredux was used
        ...
        embed_query = reducer.transform(vec_query)

        with tables.open_file(args.pytable, 'a') as db:
            embed_all = np.vstack((db.root.embed_x[:], db.root.embed_y[:])).T
        no_of_objs = embed_all.shape[0]
        ind = []
        hits = []
        hits_dist = []
        if nneighbors >= no_of_objs:
            nneighbors = no_of_objs-1
            print(f"User: Nneighbors is larger than no of objects in project. Changed it to {nneighbors}",
                  flush=True)

        dists = cdist(embed_query, embed_all, metric='euclidean').squeeze()
        ind = np.argpartition(dists, nneighbors)[0:nneighbors]
        if len(ind):
            hits = ind[np.argsort(dists[ind])]
            if len(hits): # return these two values
