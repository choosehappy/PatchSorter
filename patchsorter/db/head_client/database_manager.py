    ...
    def setup_triggers(self, project_id: int, raw_conn) -> None:
        ...
        insert_trigger_fn_sql = f"""
            ...
            BEGIN
                ...
                -- e.g. 'project{n}_pred_patch_latest_102008' -> 102008
                ...
                IF v_pred_last_regclass IS NOT NULL AND v_pred_last_shard IS NULL THEN
                    RAISE EXCEPTION 'could not resolve project{n}_pred_patch_last shard for pred shardid %', v_pred_shardid;
                END IF;
        ...
        update_trigger_fn_sql = f"""
            ...
            BEGIN
                ...
                IF v_pred_last_regclass IS NOT NULL AND v_pred_last_shard IS NULL THEN
                    RAISE EXCEPTION 'could not resolve project{n}_pred_patch_last shard for patch shardid %', v_patch_shardid;
                END IF;
