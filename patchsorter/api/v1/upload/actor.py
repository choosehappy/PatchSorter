...
class UploadSessionActor:
    ...
    def save_images(self, filenames: List[str], contents: List[bytes]) -> str:
        dest = os.path.join(self._tmpdir.name, "images")
        for name, data in zip(filenames, contents):
            with open(os.path.join(dest, os.path.basename(name)), "wb") as f:
                f.write(data)
        return f"Uploaded {len(filenames)} image(s)"

    def save_masks(self, filenames: List[str], contents: List[bytes]) -> str:
        dest = os.path.join(self._tmpdir.name, "masks")
        for name, data in zip(filenames, contents):
            with open(os.path.join(dest, os.path.basename(name)), "wb") as f:
                f.write(data)
        return f"Uploaded {len(filenames)} mask(s)"

    def save_labels(self, filenames: List[str], contents: List[bytes]) -> str:
        dest = os.path.join(self._tmpdir.name, "labels")
        for name, data in zip(filenames, contents):
            with open(os.path.join(dest, os.path.basename(name)), "wb") as f:
                f.write(data)
        return f"Uploaded {len(filenames)} label file(s)"
    ...
    def validate_paths(self, paths: list[dict]) -> dict:
        ...
        """
        images_dir = os.path.join(self._tmpdir.name, "images")
        masks_dir = os.path.join(self._tmpdir.name, "masks")
        labels_dir = os.path.join(self._tmpdir.name, "labels")
