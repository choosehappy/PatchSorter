...
def add_async_job(project_id, command, parameters, arguments, what_to_run, imageid=None, callback=None):
    ...
    meta = psdb.db.metadata
    newjobtable = JobidBase.__table__.tometadata(meta, name=jobtablename)
    newjobtable.create(bind=psdb.db.engine, checkfirst=True)

    Base = automap_base()
    Base.prepare(psdb.db.engine, reflect=True)
    ...
    psdb._pool.apply_async(what_to_run, args=arguments_and_job,
