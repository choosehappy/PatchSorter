...
def _apply_borders(img_t: torch.Tensor,
...
def _apply_borders_batch(imgs: torch.Tensor,
...
def log_nearest_neighbors(writer, img_aug, orig, proj_emb, proj_coords, niter_total,
    ...
    def make_grid(sim, use_aug_query, use_aug_neighbors):
        ...
        if use_aug_query:     q_imgs  = _pad_to(q_imgs,  H, W)
        if use_aug_neighbors: nn_imgs = _pad_to(nn_imgs, H, W)
        ...
        for qi in query_idx:
            ...
            rows.append(row)
        return vutils.make_grid(torch.cat(rows, dim=0),
                                nrow=n_neighbors + 1, padding=2, normalize=False)
...
def log_nearest_neighbors_orig(writer, orig, sim_emb, sim_coords, niter_total,
                                labels=None, pred_labels=None,
                                n_queries=5, n_neighbors=5):
    """
    labels      : int tensor [B], gt labels (-1 = unlabeled). Drives inner border.
...
def log_embeddings(writer, z_batch, proj_coords, pred_logits, labels, pred_labels, high_conf,
...
# ---- 4. projected 2D coordinates as a scatter image
    ...
    if unlabeled_mask.any():
        ...
        if low_conf_mask.any():
            colors = [_label_to_color(int(l), lightness=0.3) for l in pred_labels[low_conf_mask].cpu()]
            ax.scatter(coords_np[low_conf_mask.cpu(), 0],
                       coords_np[low_conf_mask.cpu(), 1],
                       c=colors, alpha=0.2, s=10)

        if high_conf_mask.any():
            colors = [_label_to_color(int(l), lightness=0.6) for l in pred_labels[high_conf_mask].cpu()]
            ax.scatter(coords_np[high_conf_mask.cpu(), 0],
                       coords_np[high_conf_mask.cpu(), 1],
                       c=colors,alpha=0.5, s=15)

    if labeled_mask.any():
        colors = [_label_to_color(int(l), lightness=1.0) for l in labels[labeled_mask].cpu()]
        ax.scatter(coords_np[labeled_mask.cpu(), 0],
                   coords_np[labeled_mask.cpu(), 1],
                   c=colors,alpha=0.85, s=30,marker='x')
