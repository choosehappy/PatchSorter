...
class RandomPatchDataset(Dataset):
    ...
    def __init__(self, total: int, patch_size: int = 64, pool_size: int = 8192):
        ...
        self.pool = torch.rand(pool_size, 3, patch_size, patch_size, dtype=torch.float32)
...
def run_benchmark(args) -> dict:
    ...
    if RICH:
        ...
        console.print(Panel("\n".join(info_lines), title="[bold white]⚡ GPU Patch Benchmark[/]",
    ...
    if RICH:
        with console.status("[yellow]Warming up GPU…"):
            warmup(model, device, args.batch_size, args.patch_size,
                   steps=args.warmup_steps, use_amp=use_amp)
    else:
    ...
    if RICH:
        with Progress(
            ...
            task = progress.add_task(
                "Processing patches", total=n_batches, rate="–"
            )
        ...
        for i, batch_cpu in enumerate(loader):
            ...
            if (i + 1) % report_every == 0 or (i + 1) == n_batches:
                ...
                print(f"  [{pct:5.1f}%]  {total_patches:>10,} patches  |  {rate:>10,} patches/s")
            t_dl_start = time.perf_counter()
...
def display_results(r: dict):
    if RICH:
        ...
        console.print(Panel(tput, title="[bold white]🚀 Throughput", border_style="bright_green", padding=(0,2)))

        # ── latency table ─────────────────────────────────────────────────────
        lat_table = Table(box=box.SIMPLE_HEAVY, show_header=True,
                          header_style="bold bright_blue", min_width=40)
        ...
        console.print(Panel(lat_table, title="[bold white]⏱  Batch Latency",
                            border_style="bright_blue", padding=(0,2)))

        # ── time breakdown ────────────────────────────────────────────────────
        w = r['t_wall_s']
        def pct(t): return 100 * t / w if w > 0 else 0

        br_table = Table(box=box.SIMPLE_HEAVY, show_header=True,
                         header_style="bold magenta", min_width=50)
        ...
        br_table.add_row("DataLoader (CPU prefetch)", f"{r['t_dataloader_s']:.3f}", f"{pct(r['t_dataloader_s']):.1f}%")
        br_table.add_row("Host → Device (H2D)",       f"{r['t_h2d_s']:.3f}",       f"{pct(r['t_h2d_s']):.1f}%")
        br_table.add_row("GPU Forward Pass",           f"{r['t_forward_s']:.3f}",   f"{pct(r['t_forward_s']):.1f}%")
        br_table.add_row("Device → Host (D2H)",        f"{r['t_d2h_s']:.3f}",       f"{pct(r['t_d2h_s']):.1f}%")
        console.print(Panel(br_table, title="[bold white]🔬 Time Breakdown",
        ...
        console.print(Panel(hint, title="[bold white]💡 Analysis", border_style="yellow", padding=(0,2)))
...
def parse_args():
    ...
    p.add_argument("--workers",      type=int, default=auto_workers(),
    ...
    p.add_argument("--cpu",          action="store_true",
                   help="Force CPU (useful to baseline or debug)")
...
RECOMMENDED_MODELS = [
    ...
    ("vit_small_patch16_224",    "ViT — note: expects 224×224 input, will resize internally"),
...
def main():
    ...
    if args.list_models:
        ...
        print(f"Full list: python -c \"import timm; print('\\n'.join(timm.list_models()))\"")
        return
