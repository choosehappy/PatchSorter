...
)  # ~0.1024  #--- i think this may need to be a positive number?
    ...
    for batch_idx, batch_data in tqdm(enumerate(dataloader)):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=True):
            *views, labels, orig = batch_data
            ...
            semantic_coord_attr_loss, semantic_coord_repel_loss = semantic_head_loss(
                proj_coords, labels
            )
            semantic_coord_loss = (
                semantic_coord_attr_loss + semantic_coord_repel_loss
            )  # report seperately

            proj_emb_norm = F.normalize(
                proj_emb, dim=1
            )  # projects onto unit hypersphere
            semantic_emb_attr_loss, semantic_emb_repel_loss = semantic_head_loss(
                proj_emb_norm, labels, margin=0.5
            )
            semantic_emb_loss = (
                semantic_emb_attr_loss + semantic_emb_repel_loss
            )  # report seperately

            class_weights = label_tracker.get_class_weights()

            sup_pred_loss = prediction_loss_sup(
                pred_logits,
                labels,
                class_weights=class_weights,
            )

            pseudo_pred_loss, pred_labels, high_conf = prediction_loss_pseudo(
                pred_logits,
                labels,
                pseudo_class_weights=None,
                pseudo_thresh=PSEUDO_THRESH,
                views_per_patch=NVIEWS,
            )  # i don't think we want psudo class weights
            pred_loss = (
