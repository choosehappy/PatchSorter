...
def inter_bin_margin_loss(coords, min_margin=MIN_INTER_BIN_MARGIN):
    ...
    for i, b in enumerate(bins):
        bin_points.setdefault(b, []).append(coords[i])
    centroids = (
        torch.stack([torch.stack(v).mean(dim=0) for v in bin_points.values()])
        if bin_points
        else torch.tensor([])
    )
    if len(centroids) < 2:
    ...
    for i in range(len(centroids)):
        for j in range(i + 1, len(centroids)):
            ...
            if d < min_margin:
                loss += (min_margin - d) ** 2
    return (
        loss / (len(centroids) * (len(centroids) - 1) / 2)
        if len(centroids) > 1
...
def streaming_training_loop(backbone, projection_head, data_loader, memory_bank):
    ...
    for batch_idx, batch_patches in enumerate(data_loader):
        ...
        if batch_idx % 10 == 0:
            print(
                f"Batch {batch_idx}: total_loss={total_loss.item():.4f}, memory_size={len(memory_bank.heap)}"
            )
