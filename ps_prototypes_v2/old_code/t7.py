...
# )
# def forward(self, z):
# return self.fc(z)  # logits, shape [B, num_classes]


class JointHead(nn.Module):
    def __init__(self, embed_dim, proj_dim, num_classes):
        super().__init__()
        self.shared_fc = nn.Sequential(
            nn.Linear(embed_dim, 128), nn.ReLU(), nn.Linear(128, 128), nn.ReLU()
        )
        self.proj_fc = nn.Sequential(
            nn.Linear(128, proj_dim),
            nn.Sigmoid(),  # then scale by GRID_SIZE
        )
        self.pred_fc = nn.Linear(128, num_classes)

    def forward(self, z):
        shared = self.shared_fc(z)
        proj = self.proj_fc(shared) * GRID_SIZE
...
class MemoryBank:
    ...
    def sample(self, k):
        if self.z.shape[0] == 0:
            return torch.empty((0, self.z.shape[1]), device=DEVICE), torch.empty(
                (0, 2), device=DEVICE
            )
...
def importance_score_tensor(bins, labels, age_tensor, epsilon=1e-3):
    ...
    flat_bins = (
        bins_tensor[:, 0] * GRID_SIZE + bins_tensor[:, 1]
    )  # flatten 2D -> 1D index
    counts = torch.bincount(
        flat_bins, minlength=GRID_SIZE * GRID_SIZE
    )  # [GRID_SIZE*GRID_SIZE]
...
def prediction_loss(logits, labels, pseudo_thresh=0.95):
    """
    logits: [B, C]
    labels: [B] tensor (-1 = unlabeled)
    pseudo_thresh: confidence threshold for pseudo-labeling
    """
    device = logits.device
    B, C = logits.shape
    probs = F.softmax(logits, dim=1)

    # 1. Supervised loss for labeled points
    ...
    # 2. Pseudo-labeling for high-confidence unlabeled points
    unlabeled_mask = labels < 0
    if unlabeled_mask.any():
        unlabeled_probs = probs[unlabeled_mask]
        max_conf, pseudo_labels = torch.max(unlabeled_probs, dim=1)
        high_conf_mask = max_conf >= pseudo_thresh
        if high_conf_mask.any():
            pseudo_loss = F.cross_entropy(
                logits[unlabeled_mask][high_conf_mask], pseudo_labels[high_conf_mask]
            )
        else:
    ...
    return sup_loss + pseudo_loss_lambda * pseudo_loss
...
def neighborhood_loss(z_batch, proj_coords, k=K_NEIGHBORS):
    ...
    with torch.no_grad():
        ...
        _, idx = torch.topk(
            dist, k=k + 1, largest=False
        )  # nearest neighbors (including self)
    # exclude self
    idx = idx[:, 1:]
    loss = 0.0
    for i in range(z_batch.shape[0]):
        xi = proj_coords[i]
        neighbors = proj_coords[idx[i]]
...
def streaming_training_loop(backbone, head, loader, mem_bank):
    ...
    for batch_idx, batch, labels_batch in enumerate(loader):
        z_batch = backbone(batch.to(DEVICE))
        labels_batch = labels_batch.to(DEVICE)

        proj_coords, pred_logits = joint_head(z_batch)

        # proj_coords = head(z_batch)
        bins_batch = assign_bins(proj_coords)

        # compute losses
        mem_z, mem_coords = mem_bank.sample(MEMORY_SAMPLE_SIZE)
        loss_temp = (
            temporal_loss(mem_coords, proj_coords[: mem_coords.shape[0]])
            * TEMPORAL_LAMBDA
        )
        occ_loss, intra_loss, inter_loss = bin_losses_vectorized(
            proj_coords, target_count
        )
        neigh_loss = neighborhood_loss(z_batch, proj_coords) * NEIGHBOR_LAMBDA

        total_loss = (
            BATCH_BIN_LAMBDA * occ_loss
            + INTRA_BIN_LAMBDA * intra_loss
            + INTER_BIN_LAMBDA * inter_loss
            + neigh_loss
            + loss_temp
        )

        semantic_loss = semantic_head_loss(proj_coords, labels_batch)
        total_loss += SEMANTIC_LAMBDA * semantic_loss

        # 1. Compute predictions
        pred_logits = pred_head(z_batch)

        # 2. Compute supervised + pseudo-label loss
        pred_loss = prediction_loss(pred_logits, labels_batch)
        ...
        # update memory bank
        mem_bank.add_candidates(z_batch.detach(), proj_coords.detach(), bins_batch)

        if batch_idx % 10 == 0:
            print(
                f"Batch {batch_idx}: loss={total_loss.item():.4f}, memory={mem_bank.z.shape[0]}"
            )
...
if __name__ == "__main__":
    ...
    class DummyBackbone(nn.Module):
        def forward(self, x):
            return torch.randn(x.shape[0], EMBED_DIM, device=DEVICE)
