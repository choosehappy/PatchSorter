...
def prediction_loss(logits, labels, pseudo_thresh=0.95):
    ...
    if unlabeled_mask.any():
        ...
        if high_conf_mask.any():
            pseudo_loss = F.cross_entropy(logits[unlabeled_mask][high_conf_mask],
...
def streaming_training_loop(backbone, head, loader, mem_bank):
    ...
    for batch_idx, batch,labels_batch in enumerate(loader):
        ...
        loss_temp = temporal_loss(mem_coords, proj_coords[:mem_coords.shape[0]]) * TEMPORAL_LAMBDA
        occ_loss, intra_loss, inter_loss = bin_losses_vectorized(proj_coords,target_count)
