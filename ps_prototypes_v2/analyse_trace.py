...
def analyze_trace(file_path, top_n=15):
    ...
    # These are the background/system operations we want to ignore
    SYSTEM_NOISE = [
        'threading.py',
        'multiprocessing',
        'queue.py',
    ...
    print("Parsing events and filtering noise...")
    for event in events:
        if 'ts' in event and 'dur' in event:
            ...
            if 'Memcpy HtoD' in name or 'Host -> Device' in name:
                ...
                # Filter out the threading and system noise
                if any(noise in name for noise in SYSTEM_NOISE):
                    continue
    ...
    print("\n" + "="*50)
    print(" PyTorch Trace Profiler Summary (CLEANED) ")
    print("="*50)
    ...
    print(f"\n--- Top {top_n} Longest Running Operations ---")
