...
from .PS_api_image_helper import (
    delete_output_image,
    get_image,
    get_image_thumb,
    get_imagelist,
...
class Images(Resource):
    @api_ns_image.doc(
        params={"project_name": "Which project?", "image_id": "Id of Image"}
    )
    ...
    @api_ns_image.doc(
        params={"project_name": "Which project?", "file": "Path for the image file?"}
    )
...
@api_ns_image.route(
    "/<string:project_name>/image/<string:image_name>/thumbnail",
    endpoint="get_image_thumb",
)
@api_ns_image.doc(
    params={
...
@api_ns_image.route(
    "/<string:project_name>/image_folder", endpoint="upload_image_folder"
)
@api_ns_image.doc(
    params={
        "project_name": "Which project?",
        "folder_path": "path for folder to be uploaded",
    }
)
...
@api_ns_image.route(
    "/<string:project_name>/image_output/<string:image_name>",
    endpoint="get_output_image_pred",
)
@api_ns_image.route(
    "/<string:project_name>/image_output/<string:image_name>",
    endpoint="get_output_image_gt",
)
...
def delete_image(project_name, image_name):
    ...
    selected_image = (
        db.session.query(Image).filter_by(projId=proj.id, img_name=image_name).first()
    )
    if selected_image:
...
def get_image_stats(project_name, image_name):
    ...
    image = (
        db.session.query(
            Image.id, Image.img_name, Image.mask_path, Image.csv_path, Image.img_path
        )
        .filter(Image.projId == project.id)
        .filter(Image.img_name == image_name)
        .first()
    )
    ...
    with mutex:
        with tables.open_file(pytableLocation, mode="r") as hdf5_file:
            ...
            gtcounts = Counter(
                hdf5_file.root.ground_truth_label[idx][idx_anno].tolist()
            )
