...
from .PS_db import Image, MaskTypeEnum, Project, db

upload_modal = Blueprint(
    "upload", __name__, static_folder="static", template_folder="templates"
)
...
def get_modal(project_name):
    ...
    data = {
        ...
        "modal": render_template("image_upload_modal.html", project=project),
    }
    return data
...
def validate_paths(project_name, session_id):
    ...
    if is_list:
        paths, errors = _get_list_paths(
            project_name, session_id, request_data["list"][0]
        )
    else:
        paths, errors = _get_image_paths(request_data, session_id)

    # prepare the output
    data = {"paths": paths, "errors": errors, "list": is_list}
    ...
    try:
        with open(list_path) as csv_file:
            ...
            for row in csv_reader:
                ...
                output_item = {"status": "READY TO IMPORT", "from_dropzone": False}

                def _append_filename(upload_type, filename, output_item):
                    if os.path.isfile(filename):
                        if extension_error := _invalid_extension_error(
...
def _get_image_paths(upload_paths, session_id):
    ...
    def _get_files_in_dir(directory, upload_type):
        all_files = list(
            map(
                lambda filename: os.path.abspath(os.path.join(directory, filename)),
                next(os.walk(directory))[2],
            )
        )
        only_valid_extensions = [
            filename
            for filename in all_files
            if not _invalid_extension_error(upload_type, filename)
        ]
    ...
    for upload_type in upload_types:
        ...
        if (upload_paths[upload_type + is_folder_suffix]) and len(
        ...
        available_filenames[upload_type] = (
            input_filenames[upload_type].copy()
            if len(input_filenames[upload_type]) > 0
            else []
        )
        ...
        for filename in input_filenames[upload_type]:
            ...
            if is_duplicate:
                ...
                continue
            output_item = {upload_type: filename, "status": status, "error": error}
            output_data.append(output_item)
            error_count += 1
    ...
    for upload_type in ["mask", "csv"]:
        ...
        current_app.logger.info(
            f"Extracting stems done in {round(perf_counter()-start_time,1)} seconds"
        )
        ...
        current_app.logger.info(
            f"Comparing filenames done in {round(perf_counter()-start_time,1)} seconds"
        )
        ...
        for _ in range(distances.size):
            ...
            try:
                ...
                min_row, min_col = np.unravel_index(
                    np.nanargmin(distances), distances.shape
                )
            except ValueError:
        ...
        for filename in available_filenames[upload_type]:
            ...
            error_count += 1
        current_app.logger.info(
            f"Matching filenames done in {round(perf_counter()-start_time,1)} seconds"
        )

    # convert to output format
    for image_filename, extra_data in output_dictionary.items():
        output_item = {"image": image_filename, "status": "READY TO IMPORT"}
...
@upload_modal.route(
    "/upload/<project_name>/<session_id>/<upload_type>/", methods=["POST"]
)
def upload(project_name, session_id, upload_type):
    ...
    output_subfolder = _get_session_upload_subfolder(
        project_name, session_id, upload_type
    )
...
def _validate_index_mask(path):
    ...
    if skimage.measure.label(mask).max() != len(skimage.measure.regionprops(mask)):
        raise ValueError(
            f"Validation check failed (skimage.measure.label(mask).max() == len(skimage.measure.regionprops(mask)))"
        )
...
def _validate_labeled_mask(path, project_name):
    ...
    if num_unique_colors > num_classes:
        raise ValueError(
            f"Project {project_name} can only support {num_classes} classes, but {path} has {num_unique_colors} unique labels."
        )
...
def _validate_binary_mask(path):
    ...
    if len(uniques) != 2:
        raise ValueError(
            f"For binary masks, we expect exactly 2 unique values (zero and non-zero). {path} has {len(uniques)} unique values."
        )
    if uniques[0] != 0:
        raise ValueError(
            "Binary masks expect at least some pixels to have a value of zero. All your pixels have values {uniques}"
        )
...
def _validate_quick_annotator_mask(path):
    ...
    if not (np.all(uniques == [0, 255]) or np.all(uniques == [False, True])):
        raise ValueError(
            f"Quick Annotator masks must either be a binary mask or have exactly two unique values: 0 and 255, but {path} has {len(uniques) if len(uniques)>2 else uniques}"
        )
    ...
    if mask.ndim == 3:
        ...
        check_valid_rgb = all(
            rgb_code in valid_rgb_codes for rgb_code in unique_rgb_codes
        )
        if check_valid_rgb is False:
            raise ValueError(
                f"Invalid rgb_code in mask, valid rgb_codes are [0, 0, 0], [0, 0, 255], [0, 255, 255], [255, 0, 255]"
            )
    """
...
def _check_valid_mask(path, width, height, mask_type, project_name):
    ...
    if mask_width != width or mask_height != height:
        raise ValueError(
            f"[Invalid Size] Mask size was {mask_width}×{mask_height} (w × h) but the image size was {width}×{height}!"
        )
...
def _move_uploaded_file(
    project_name, session_id, upload_type, filename, files_to_roll_back
):
    session_subfolder = _get_session_upload_subfolder(
        project_name, session_id, upload_type
    )
    # current_app.logger.debug(f'session_subfolder = {session_subfolder}')
...
def _process_upload(
    paths, project_id, project_name, session_id, from_dropzone, mask_type
):
    mask_path = None
    csv_path = None
    ...
    try:
        ...
        db.session.rollback()
        raise ValueError(
            f"An image named {image_name} already exists for project {project_name}. No duplicates allowed."
        )
...
def process_uploads(project_name, session_id):
    ...
    from_dropzone = {}
    for upload_type in ["image", "mask", "csv"]:
        from_dropzone[upload_type] = (not request_data["list"]) and (
            not request_data[upload_type + "_is_folder"]
        )
    ...
    for i, (current_input_paths, output_paths) in enumerate(
        zip(input_paths, output_data)
    ):
