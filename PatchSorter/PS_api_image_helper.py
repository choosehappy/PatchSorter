...
from .PS_pool import pool_get_image, update_completed_job_status
...
def get_image(project_name, image_id):
    ...
    if img:
        ...
        response = make_response(
            jsonify(error=f"Image id {image_id} doesn't exist"), 400
        )
...
def get_imagelist(project_name, pageNo, pageLimit):
    ...
    images = (
        db.session.query(Image.id, Image.projId, Image.img_name, Image.img_path)
        .filter(Image.projId == proj.id)
        .order_by(Image.id.asc())
        .paginate(page=pageNo, per_page=pageLimit)
    )
    ###
    # Since SqlAlchemy object is not serializable a direct jsonify does not work hence making a seperate oject.
    ####
    imgList = [
        {"image_id": img.id, "image_name": img.img_name, "image_path": img.img_path}
...
def get_image_thumb(project_name, image_name):
    ...
    response.headers[
        "Content-Disposition"
    ] = f'inline; filename = "{image_name.replace(".png", "_thumb.png")}"'
    return response
...
def upload_mask(project_name, image_path=None, mask_path=None):
    ...
    imgname = filename.replace(
        "_mask", ""
    )  # this allows for accepting both \mask\train_1.png as well as train_1_mask.png
    if image_path is None:
        ...
        if mask_fold in basepath:
            ...
            image_path = mask_path.replace(
                "_mask", ""
            )  # Needed to query for Image based on path, will help if two images of the same name are present in different paths.
    img = (
        db.session.query(Image)
        .filter_by(projId=proj.id, img_name=imgname, img_path=image_path)
        .first()
    )
    # Error Handling : if img is none, need to throw an error visible to the user
    if not img:
        current_app.logger.warn(
            f"ERROR:Mask not uploaded,img for {imgname} doesn't exists in DB"
        )
        return make_response(
            jsonify(error=f"img for {imgname} doesn't exist in DB"), 404
        )
...
def upload_image(project_name, abs_file_path=None):
    ...
    if not abs_file_path:  # assume request
        ...
        dest = (
            f".{os.path.sep}projects{os.path.sep}{project_name}{os.path.sep}{filename}"
        )
        import_type = "drop"
...
def upload_image_folder(project_name):
    ...
    if (
        os.path.exists(abs_dir_path)
        and os.path.isfile(abs_dir_path)
        and abs_dir_path.endswith(".csv")
    ):
    ...
    if os.path.exists(abs_dir_path) and os.path.isdir(abs_dir_path):
        for filename in glob.glob(abs_dir_path + os.sep + "**", recursive=True):
            if not filename.lower().endswith(
                (".png")
            ):  # can eeasily extend to other datatypes here #, '.jpg', '.jpeg', '.tiff', '.bmp', '.gif')):  #--- need to upload images first
                continue
...
def get_mask(project_name, image_id, image_name):
    ...
    if img.mask_path:
        ...
        response = make_response(
            jsonify(
                error=f"Mask for Image '{image_name}' doesn't exist, Please upload a Mask."
            ),
            400,
        )
        response.headers["Content-type"] = "application/json"
...
def delete_output_image(project_name, image_id, image_name, type=None):
    ...
    full_fname = (
        f"{outdir}/{type}/overlay/{image_name}"
        if overlay
        else f"{outdir}/{type}/{image_name}"
    )
...
def get_output_image(project_name, image_id, image_name, output_type=None):
    ...
    if project.embed_iteration <= -2 and output_type == "pred":
        current_app.logger.warn(
            f"Project {project_name} needs to be embedded. Returning HTML response code 400."
        )
        return make_response(
            jsonify(
                error=f"Project {project_name} not embedded. Please Make Patches and Embed Patches."
            ),
            400,
        )
    ...
    csvfileName = (
        f"./projects/{project_name}/temp/output_details_{secrets.token_urlsafe(10)}.csv"
    )
    with open(csvfileName, "w") as f:
        image = (
            ...
            .filter(Image.projId == project.id)
    ...
    outdir = f"./projects/{project_name}/"
    imgoutput_script_name = config.get(
        "image_output",
        "imgoutput_script_name",
    ...
    full_fname = (
        f"{outdir}/{output_type}/overlay/{image_name}"
        if overlay
        else f"{outdir}/{output_type}/{image_name}"
    )
    ...
    return pool_get_image(
        project_name,
        command_name,
        full_command,
        full_fname,
        imageid=image.id,
        callback=make_output_callback,
    )
...
def get_output_overlayids(
    project_name, image_id, image_name, output_type, overlay_type
):
    current_app.logger.info(
        f"Getting overlay_ids {project_name} and image {image_name}"
    )
    ...
    if not os.path.isfile(image_filename):
        return make_response(
            jsonify(error=f"Project output has not been generated does not exist"), 400
        )
    else:
