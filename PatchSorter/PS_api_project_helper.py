...
def make_patches(project_name):
    ...
    if project is None:
        ...
        return make_response(
            jsonify(error=f"Project {project_name} does not exist"), 400
        )
    ...
    with open(csvfileName, "w", newline="") as f:
        ...
        for image in images:
            if image.make_patches_time:
                continue
            details = {
                "mask_type": image.mask_type,
                "image_id": image.id,
                "img_path": image.img_path,
                "mask_path": image.mask_path,
                "csv_path": image.csv_path,
            }
            w.writerow(details)
    ...
    makepatch_script_name = config.get(
        "make_patches",
        "makepatch_script_name",
    ...
    return pool_run_script(
        project_name, command_name, full_command, callback=make_patches_callback
    )
...
def make_patches_callback(result):
    ...
    dbretval = (
        engine.connect()
        .execute(f"select procout from jobid_{jobid} where procout like 'RETVAL:%'")
        .first()
    )
    if dbretval is None:
    ...
    for img in retvaldict["image_list"]:
        engine.connect().execute(
            f"update image set make_patches_time = datetime('now') where img_path= :img",
            img=img,
        )
    ...
    if retval == 0:
        ...
        projid = (
            engine.connect()
            .execute(f"select projId from job where id = :jobid", jobid=jobid)
            .first()[0]
        )
        engine.connect().execute(
            f"update project set make_patches_time = datetime('now') where id = :projid",
            projid=projid,
        )
...
def train_dl(project_name):
    ...
    # get config properties:
    train_script_name = config.get(
        "train_dl",
        "train_script_name",
        fallback="./approaches/simclr/train_dl_simclr.py",
    )
    num_epochs = config.getint("train_dl", "numepochs", fallback=1000)
    num_epochs_earlystop = config.getint(
        "train_dl", "num_epochs_earlystop", fallback=-1
    )
    numepochs_inital = config.getint("train_dl", "numepochs_inital", fallback=100)
    minlossearlystop = config.getfloat(
        "train_dl", "minimum_loss_earlystop", fallback=0.001
    )
    ...
    if prev_model_init:
        prev_model_fname = glob.glob(
            f"./projects/{project_name}/models/{current_modelid}/best*.pth"
        )
        if prev_model_fname:
    ...
    return pool_run_script(
        project_name, command_name, full_command, callback=train_dl_callback
    )
...
def make_embed(project_name):
    ...
    if proj.train_dl_time is None and proj.iteration == 0:
        error_message = (
            f"The base model 0 of project {project_name} was overwritten when Retrain Model 0 started.\n "
    ...
    if not os.path.isfile(pytablefile):
        return make_response(
            jsonify(error="pytable file does not exist, consider making patches first"),
            400,
        )

    modelid = request.args.get(
        "modelid", default=get_latest_modelid(project_name), type=int
    )
    outdir = f"./projects/{project_name}/models/{modelid}"

    embed_script_name = config.get(
        "embed",
        "embed_script_name",
        fallback="./approaches/simclr/make_embed_simclr.py",
    )
    ...
    return pool_run_script(
        project_name, command_name, full_command, callback=make_embed_callback
    )
...
def update_label_names(project_name, labels):
    current_app.logger.info(f"Updating Labels. Project = {project_name}")
    # labelList = json.loads(request.args.get("labels"))
    labelList = labels

    for newlabel in labelList:
        labels = (
            db.session.query(Labelnames.id, Labelnames.label_id, Labelnames.label_name)
            .filter(Labelnames.id == newlabel["id"])
            .first()
        )
        if labels is None:
            return make_response(
                jsonify(error=f"labels {project_name} doesn't exist"), 400
            )
        if labels.label_name != newlabel["label_name"]:
            labelUpdate = (
                db.session.query(Labelnames)
                .filter_by(id=newlabel["id"])
...
def get_project_status(project_name):
    ...
    return jsonify(
        ...
        percent_annotated=annotated_percent,
...
def get_job_status(job_id):
    ...
    if job is None:
        current_app.logger.warn(
            f"Unable to find {job_id} in database. Returning HTML response code 400."
        )
...
def get_points_on_grid(
    project_name, xmin, xmax, ymin, ymax, plot_by, filter_by, class_by
):
    ...
    maxpoints = request.args.get(
        "maxpoints",
        default=int(config.get("embed", "maxpoints", fallback=5000)),
        type=int,
    )
    totalres, providedres, embedding, timing = ranged_embed(
        project_name,
        round(xmin, 4),
        round(ymin, 4),
        round(xmax, 4),
    ...
    current_app.logger.debug(
        (f"Time to retrieve nearest points on grid: {datetime.now() - start_pts_prc1}")
    )
    ...
    current_app.logger.debug(
        (f"DateTime to organize data for frontend: {datetime.now() - start_pts_prc2}")
    )
...
def getHits(tree, xmin, ymin, xmax, ymax):
    ...
    current_app.logger.info(
        f"----->> Time-(getHits) tree_query is {tree_query} seconds"
    )
...
# @SearchCache.memoize()
def ranged_embed(
    project_name, xmin, ymin, xmax, ymax, plot_by, filter_by, class_by, maxpoints
):
    current_app.logger.info(
        f"Search min is {xmin}, {ymin}, and max is {xmax}, {ymax} and maxpoints {maxpoints}"
    )
    ...
    current_app.logger.info(
        f"Time-(ranged_embed) pytable_query is {pytable_query} seconds"
    )

    current_app.logger.info(
        f"Totalres is {totalres},providedres {providedres}, and embedding is {len(embedding)}"
    )

    timing = {
        "get_tree": gettree,
        "mutex_treequery": tree_query,
        "mutex_pytable": pytable_query,
    }
...
def make_searchdb(project_name):
    ...
    searchdb_script_name = config.get(
        "search_db",
        "searchdb_script_name",
        fallback="./approaches/search_db/make_search_database.py",
    )
    # get the command:
    full_command = [
        sys.executable,
        f"{searchdb_script_name}",
        f"{pytablefile}",
        f"{searchdbfile}",
    ]
    # run the command asynchronously
    ...
    return pool_run_script(
        project_name, command_name, full_command, callback=make_searchdb_callback
    )
...
def delete_label(project_name, label_id):
    ...
    selected_label = (
        db.session.query(Labelnames)
        .filter_by(projId=proj.id, label_id=label_id)
        .first()
    )
