...
from .PS_db import Project, db, get_latest_modelid
...
@api_ns_search.route(
    "/<string:project_name>/search/upload_image_search", endpoint="upload_image_search"
)
class UploadSearchImage(Resource):
    @api_ns_search.doc(params={"project_name": "Which project?"})
    def post(self, project_name):
        ...
        if file.filename == "" or not file.filename.endswith(
            "png"
        ):  # assume only png, should be expanded to include reasonable formats, jpg, tif, others?
            return jsonify("error")  # put reasonable error message here

        image_fname = os.path.join(
            "projects", project_name, "image_search", file.filename
        )
...
@api_ns_search.route(
    "/<string:project_name>/search/image_search", endpoint="image_search"
)
class ImageSearch(Resource):
    @api_ns_search.doc(
        params={
            ...
            "max_threshold": "Max threshold for positive result",
        }
    )  # add modelid
    def post(self, project_name):
        proj = db.session.query(Project).filter_by(name=project_name).first()
        ...
        filename = (
            file.filename
        )  # request gives empty file object with filename '' if no file selected
        ...
        try:
            ...
            nneighbors = int(
                request.form.get(
                    "nneighbors", config.get("image_search", "nneighbors", fallback=100)
                )
            )
        except ValueError:
            return make_response(
                jsonify(error="Threshold and Nneighbors have to be numbers"), 400
            )

        patch_size = config.getint("image_search", "patchsize", fallback=32)
        model_name = (
            f"./projects/{project_name}/models/{get_latest_modelid(project_name)}/"
        )
        ...
        if not os.path.isfile(pytablefile):
            return make_response(
                jsonify(
                    error="pytable file does not exist, consider making patches first"
                ),
                400,
            )

        # --- figure out which key
        approach_script_name = config.get(
            "image_search",
            "approach_options",
            fallback="embed:Embedding:./approaches/image_search/image_search_embed.py",
        )
        approaches = (
            approach_script_name.splitlines()
        )  # this part should really be done by converting the list to a dict, and then accesing tuple by key
        keys = [a.split(":")[0] for a in approaches]
        approach = request.form.get(
            "approach",
        ...
        try:
            if approach not in keys:
                current_app.logger.info(
                    f"Approach {approach} is not found in config.ini using default approach"
                )
                approach = config.get(
                    "image_search", "approach_default", fallback="embed"
                )
            idx = keys.index(approach)  # issue failure here if not in list
        except ValueError:
            return make_response(
                jsonify(error="Error finding search patches approach"), 400
            )
        ...
        dbretval_result = subprocess.run(
        ...
        if "RETVAL" in result:
            ...
            if len(hits):
                ...
                return make_response(
                    jsonify(embeddingCnt=len(hits), embeddingKey=embedkey), 200
                )
            else:
