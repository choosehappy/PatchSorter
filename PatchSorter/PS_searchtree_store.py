...
class PS_SearchTreeStore:
    ...
    def __getitem__(self, key):
        ...
        modificationTime = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(os.path.getmtime(searchdbfile))
        )
        ...
        current_app.logger.info(
            f"SearchTree_Store mutex: {mutex_exit - mutex_int} seconds"
        )
    ...
    def __setitem__(self, key, value):
        if len(self.store) >= self.maxstoreitems:
            ...
            self.store.popitem(
                last=False
            )  # popitem with last=False uses FIFO and removes first added item in ordereddict.
        self.store[
            key
        ] = value  # moved the data storage after popitem to end up with right maxstoreitems.
...
SearchResultStore = PS_SearchResultStore(
    config.getint("caching", "searchqueries", fallback=5)
)
SearchTreeStore = PS_SearchTreeStore(
    config.getint("caching", "searchtreestorageitems", fallback=1)
)
