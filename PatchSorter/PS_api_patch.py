...
class PatchImage(Resource):
    @api_ns_patch.doc(
        params={"project_name": "Which project?", "patch_id": "Patch Id?"}
    )
...
@api_ns_patch.route(
    "/<project_name>/<patch_id>/context", endpoint="patch_context_image"
)
...
class PatchData(Resource):
    @api_ns_patch.doc(
        params={
            ...
            "patch_id": "Patch Id or Ids seperated by a comma(,)?",
        }
    )
    def get(self, project_name, patch_id):
        """returns Patch Data"""
...
class PatchByPolygon(Resource):
    @api_ns_patch.doc(
        params={"project_name": "Which project?", "polystring": "d3 string for lasso"}
    )
    def get(self, project_name):
        ...
        return patch_by_polygon(
            project_name, polygonstring, plot_by, filter_by, class_by
        )
...
@api_ns_patch.route(
    "/<string:project_name>/closest_patch/<int:image_id>/<int:X>/<int:Y>",
    endpoint="closest_patch",
)
class ClosestPatch(Resource):
    @api_ns_patch.doc(
        params={
            ...
            "X": "X query coordinate",
            "Y": "Y query coordinate",
        }
    )
