...
from .PS_db import Image, Job, Project, get_images, get_latest_modelid
...
def index():
    projects = (
        db.session.query(
            ...
            Project.description,
            Project.id,
            Project.no_of_label_type,
            Project.images,
            db.func.count(db.func.distinct(Image.id)).label("nImages"),
            db.func.ifnull(db.func.sum(db.func.distinct(Image.nobjects)), 0).label(
                "nObjects"
            ),
        )
...
def view_embed_main(project_name):
    ...
    project = (
        db.session.query(
            ...
            Project.description,
            Project.date,
            Project.make_patches_time,
            Project.no_of_label_type,
            Project.iteration,
            Project.embed_iteration,
            Project.images,
            db.func.count(db.func.distinct(Image.id)).label("nImages"),
        )
        .outerjoin(Image, Image.projId == Project.id)
...
@html.route(
    "/<project_name>/image/<image_id>/gt",
    methods=["GET"],
    endpoint="get_output_image_gt",
)
@html.route(
    "/<project_name>/image/<image_id>/pred",
    methods=["GET"],
    endpoint="get_output_image_pred",
)
@html.route(
    "/<project_name>/image/<image_id>/image", methods=["GET"], endpoint="view_image"
)
@html.route(
    "/<project_name>/image/<image_id>/mask", methods=["GET"], endpoint="view_mask"
)
