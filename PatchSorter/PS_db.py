    ...
    if job_id:
        ...
        if status == "DONE":
            engine.connect().execute(
                f"update job set status= :status, retval = :retval, end_date= datetime('now') "
                f"where id={job_id}",
                status=status,
                retval=retval,
            )
            ...
            engine.connect().execute(
                f"update job set status= :status, retval = :retval where id={job_id}",
                status=status,
                retval=retval,
            )
        engine.dispose()
...
def get_images(projectId):
    images = (
        db.session.query(
            ...
            Image.width,
            Image.date,
            Image.make_patches_time,
            Image.nobjects,
        )
        .filter(Image.projId == projectId)
        .group_by(Image.id)
        .all()
    )
...
def get_annotations_percent(ground_truth_array):
    ...
    total_count = sum(
        counts.values()
    )  # upgrade to python 3.10 and you can do counts.total()
    obj_ann_percent = {
        str(key): round(val / total_count * 100, 2) for key, val in counts.items()
    }

    if counts[-1] != total_count:  # Check to verify if all data is 100% unlabelled
        # Check to verify if all data is 100% labelled
        ann_percentage = (
            round(100 - obj_ann_percent["-1"], 1) if "-1" in obj_ann_percent else 100
        )
...
def insert_new_project(project_name, description, noLables):
    newProject = Project(
        name=project_name, description=description, no_of_label_type=int(noLables)
    )
...
def create_project_label(projectId, no_label_names):
    ...
    for cnt in range(nolabel):
        ...
        newLabel = Labelnames(
            projId=projectId, label_id=cnt, label_name=str(cnt), label_color=label_color
        )
...
def check_projectexists(project_name):
    ...
    if proj is not None:
        raise ProcessingException(
            description=f"Project {project_name} already exists.", code=400
        )
...
def get_filtered_ids(idx, plot_by, filter_by, class_by, project_name):
    ...
    current_app.logger.info(
        f"Time-(get_filtered_ids) for Mutex - {time_end_mutex-time_init_mutex}s"
    )
    ...
    filter_idx = None
    if filter_by == int(
        FilterPlot.UNLABELED
    ):  # Unlabelled patch is always one with no ground truth.
        ...
        FilterPlot.DISCORDANT
    ):  # Discordant check is just gt != pred and hence not checkin for label or class.
        ...
        filter_idx = (
            np.isin(pred, class_by)
            if plot_by == int(FilterPlot.PREDICTION)
            else np.isin(gt, class_by)
        )
    ...
    current_app.logger.info(
        f"Time-(get_filtered_ids) for filtering - {time_filter_end-time_init_mutex}s"
    )
    return idx
