...
from .PS_api_project_helper import (
    get_job_status,
    get_points_on_grid,
    get_project_status,
    getconfig,
...
class ViewEmbed(Resource):
    @api_ns_project.doc(params={"project_name": "Which project?", "maxpoints": "0"})
    def get(self, project_name):
        ...
        if proj.embed_iteration == -2:
            return make_response(
                jsonify(
                    error=f"For Graphs- 1.Embed or 2.Train DL and Embed for {project_name}."
                ),
                400,
            )
...
@api_ns_project.route(
    "/<string:project_name>/labelnames", endpoint="label_names", doc=False
)
...
@api_ns_project.route(
    "/<string:project_name>/project_status", endpoint="project_status"
)
...
@api_ns_project.route(
    "/<string:project_name>/points_on_grid", endpoint="points_on_grid"
)
